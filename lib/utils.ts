...
export const getCurrentChapter = (
  ...
  return (
    data.find(
      (chapter) => parseInt(chapter.slug.split("-")[1]) === currentChapterIndex,
    ) || null
  );
};
...
export const parseContent = (data: any): FetchContentResult => {
  ...
  contentSections.forEach((section: string) => {
    ...
    const quizElements =
      sectionDoc.querySelectorAll<HTMLElement>(".quiz-block");
      ...
      if (jsonString) {
        try {
          ...
          if (Array.isArray(jsonData) && jsonData.length > 0) {
            ...
            allQuizData[currentPageIndex] =
              allQuizData[currentPageIndex].concat(jsonData);
          }
        } catch (error) {
          console.error(
